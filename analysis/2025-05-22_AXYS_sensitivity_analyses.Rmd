...
## Load sensitivity analyses data

I generated dataframes that compare the correlation coefficients (r) before and after "correction" (e.g., using imputed values, removing one or more influential individuals). These files are in: *outputs/2024-10-02_ProjectGrant_sensitivity_analyses*.

```{r}
# Combine all correlation files into one dataframe
compiled_outputs <- suppressWarnings(
  map_dfr(.x = list.files(path = here::here("outputs", "2024-10-02_ProjectGrant_sensitivity_analyses"),
                          recursive = TRUE,
                          pattern = ".csv",
                          full.names = TRUE),
          .f = ~read_csv(.x) %>%
...
# Sample sizes per compound
compounds_n <- read_xlsx(path = here::here("data", "2024-10-03_List_of_compounds_with_sample_size.xlsx")) %>%
...
# Combine dataframe with r change values with dataframe with sample sizes per compound (may differ between tissue types)
...
## Calculate the range of change in correlation coefficient "r"
...
I performed rowwise t-tests between the initial vs new r for all comparisons to check for statistically significant differences due to one or more of the correction types.

*Correction types*: "imputation",  "removed R419", "removed_R421", "removed_R389", "removed_R448"
...
# Perform t-tests by row, then filter only significant differences (example)
...
*Note*: We did not proceed with using results from linear models for our study given our small sample sizes.
...
axys_main_corr <- read_excel(here::here("data", "2024-11-07_ComparisonList_AXYS_linear_models_adjustments.xlsx"), sheet = 1) %>%
...
Our analyses include both individual analytes and analyte sums, which are calculated in the code chunk below.
...
The following code chunks are functions for log2-transforming data and checking data characteristics (e.g., normality).

```{r}
# Function to log2 transform data
...
## Prepare the pollutant concentration data (single analytes and sums)
...
gsis_adi_all_ave <- read_excel(here::here("data", "Perifusion+GSIS", "2024-05-18_IsletCore_GSIS_from_TeamGrant_website.xlsx")) %>%
...
## Combine pollutant concentration data with GSIS parameters

```{r}
# Combine pollutant data with static GSIS data from the ADI
...
# Combine pollutant data with glucose perifusion data from JK
...
# Combine pollutant data with leucine perifusion data from JK
...
# Combine pollutant data with fatty acid perifusion data from JK
...
# Combine pollutant data with glucose perifusion data from MH
...
# Create a list with each combined dataset for pancreas and for adipose
...
# Simplify the lists into one dataset
...
## Prepare lists of compounds and functional parameters for linear models

```{r}
# Create compounds list
sum_dioxin <- data.frame(Compound = "Dioxin", Dioxin_similarity = "Dioxin")
sum_pcb <- data.frame(Compound = c("PCB", "DL", "NDL"), Dioxin_similarity = c("DL", "DL", "NDL"))
...
```{r}
# Define pollutants and functional outputs
...
## Custom functions for linear modelling and extracting summaries of results

I want to run models with no adjustments (for comparison) and with adjustments for BMI, Age, Sex, Age and BMI.
...
```{r}
# Fit all models but include comparison metrics
