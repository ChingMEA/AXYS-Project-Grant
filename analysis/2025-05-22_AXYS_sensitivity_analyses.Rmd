...
I generated dataframes that compare the correlation coefficients (r) before and after "correction" (e.g., using imputed values, removing one or more influential individuals). These files are in: *data/2024-10-02_ProjectGrant_sensitivity_analyses*.

```{r}
# Combine all correlation files into one dataframe
compiled_outputs <- suppressWarnings(
  map_dfr(.x = list.files(path = here::here("data", "2024-10-02_ProjectGrant_sensitivity_analyses"),
                          recursive = TRUE,
                          pattern = ".csv",
                          full.names = TRUE),
          .f = ~read_csv(.x) %>%
...
compounds_n <- read_xlsx(path = here::here("data", "2024-10-03_pops_analytes_list_with_sample_size.xlsx")) %>%
...
axys_main_corr <- read_excel(here::here("data", "2024-11-07_comparison_list_pops_linear_models_adjustments.xlsx"), sheet = 1) %>%
...
gsis_adi_all_ave <- read_excel(here::here("data", "2024-05-18_isletcore_static_gsis.xlsx")) %>%
